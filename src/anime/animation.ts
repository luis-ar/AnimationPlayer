...
import { parseEasings } from "./eases";
...
import { composeTween, getTweenSiblings, overrideTween } from "./compositions";
...
const generateKeyframes = (keyframes, parameters) => {
  ...
  if (isArr(keyframes)) {
    ...
    const totalDuration = /** @type {Number} */ setValue(
      parameters.duration,
...
export class Animation extends Timer {
  ...
  constructor(
    ...
    const params = /** @type {AnimationParams} */ kfParams
      ? mergeObjects(
          generateKeyframes(
    ...
    const tEasing = hasSpringEasing
      ? /** @type {SpringEasing} */ ease.solver
      : setValue(ease, animEase ? "linear" : animDefaults.ease);
    const tDuration = hasSpringEasing
      ? /** @type {SpringEasing} */ ease.duration
        ...
        if (isKey(p)) {
          ...
          if (tweenType !== tweenTypes.INVALID) {
            ...
            for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
              ...
              const tweenEasing = hasSpringEasing
                ? /** @type {SpringEasing} */ keyEasing.solver
                : keyEasing || tEasing;
              // Calculate default individual keyframe duration by dividing the tl of keyframes
              const tweenDuration = hasSpringEasing
                ? /** @type {SpringEasing} */ keyEasing.duration
                : getFunctionValue(
                    setValue(
                      key.duration,
                      l > 1
              ...
              const isFromToValue =
                isFromToArray ||
                (!isUnd(tweenFromValue) && !isUnd(tweenToValue));
    ...
    this.duration =
      clampInfinity(
        (iterationDuration + this._loopDelay) * this._iterationCount -
          this._loopDelay
      ) || minValue;
